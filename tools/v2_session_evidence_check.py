...
def check(*, session: str, window_start: str, window_end: str, catalog: Path,
    ...
    if "." not in window_end:
        ...
        # second, not just its exact zero-microsecond instant: otherwise a
        # succeeded attempt at "...T23:59:59.412000Z" -- inside the
        # documented window -- sorts above the bound in both the SQL BETWEEN
        # in _resolve_sources and the datetime comparison in _in_window, and
        # is wrongly reported uncovered. An operator who explicitly writes
        # ".000000Z" is asking for that exact instant, not the whole second,
        # so the check is on the raw string (whether a fractional part was
        # given at all), not on the parsed datetime's microsecond value.
        # Bumping end itself (rather than patching each comparison site
