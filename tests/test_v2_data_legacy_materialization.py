...
from engine.v2.data.reference_inputs import (  # noqa: E402
...
# Whole-table dependency planning preserves recorded materialization bounds.
ROW_COUNT = 9_123_661
...
def test_unsupported_query_and_snapshot_binding_are_refused(tmp_path):
    ...
    with pytest.raises(DataError, match="CONTRACT_MISMATCH"):
        lm.explain_materialization_dependencies(repo, snapshot, "daily_market", dataclasses.replace(query, snapshot_id="other"))
    with pytest.raises(DataError, match="CONTRACT_MISMATCH"):
        lm.explain_materialization_dependencies(repo, snapshot, "daily_market", dataclasses.replace(
