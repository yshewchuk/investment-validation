...
| R4-20 | Residual parity gaps after R4-10/R4-16 | Open. (1) Native drops non-finite paired residual rows; legacy `ResidualPool` drops only NaN. (2) The frozen paired artifact orders tied dates canonically, so exact tie-date parity with legacy holds only on the rows path. (3) The forecast-analog gate's derived columns (`analog_mean`, `analog_win_rate`, `analog_n`, `pred_abs_move_p10`/`_p90`/`_sd`, `forecast_edge`) have no native mapping into gate features. (4) A non-finite frozen-model feature refuses `INVALID_FEATURE` natively; legacy flags `MISSING_FEATURES`. (5) The DYN-SV chooser model has no frozen recipe |
