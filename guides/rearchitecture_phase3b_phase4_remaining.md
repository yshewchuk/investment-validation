...
| R4-13 | Frozen Phase 5 inference not integrated | Open, progressed through Phase 5. P5-1..P5-6 are merged: inventory (`9a54951`), the no-fit guard and acceptance audit (`58b7ee6`, `3dea05e`), training recipes (`926cca9`), frozen payoff, recalibration and residual artifacts (`30e8563`, `3dea05e`, `52ef989`, `f2b4d88`), staged releases with an atomic pointer (`8c90b90`), and the P5-6 gate plus Phase 4 replay against the staged release (`33bec3d`, `6ba53e5`). Branch `phase5-code-gaps` (2026-09-19) completed the rest of the code. Every catalog member now has an artifact type and a builder: the trailing `pnl_sim` cutoff was the last one. Every consumer has a probe, the last being the entry-rule gate and the Tier-4 serving folds. The P5 replay serves the frozen chooser (champion, producer folds, k-NN pool, n_admissible table) and the entry-rule cutoff from the staged release. The audit also flags unapproved `population.excluded` kinds. What is left is running the real builds and the gate (runbook §1-§3). Still emits P4-B01: `checks/phase4_real.py` sets `phase5_inference_integrated` true only when every final control passes, which needs the capture. Stays a Phase 4 blocker (decision 2) |
...
| R4-21 | The 7 entry-rule strategies had no native gate | **Closed 2026-09-19** (`cf47db3`, `75d40e0`, merged as `fc5f8db`). Found while verifying the Phase 5 consumers. When a strategy has no gate champion, legacy `Scorer._apply_entry_rule` decides the gate. This covers TWIN-P, TWIN-P5, CND-PS, BFLY-P, BFLY-P5, RAMP7 and CTR5. The rule has three terms: `exp_pnl_sim >= trailing cutoff`, `rel_spread <= 0.25` and `mcap_usd >= 10e9`. A missing term makes the verdict None (`MISSING_FEATURES`). Before this fix, v2 had no counterpart, so every scored row of those strategies would have mismatched on `verdicts`. The capture converter now declares `gate.mode = entry_rule` from the recorded `gate_inputs` checkpoint. `mcap_usd` comes from the recorded facts. The legacy `pnl_cutoff` is frozen as a `TrailingCutoffArtifact` for the event month, the same pattern as R4-18's recorded map. `exp_pnl_sim` and `rel_spread` are derived natively. The legacy capture itself is unchanged, but corpora captured before `75d40e0` carry `gate=not_applicable` and must be recaptured. Tests: `tests/test_v2_scoring_native_entry_rule.py` covers the real `_apply_entry_rule` over 13 scenarios for all 7 strategies, the capture to `phase4_real` round trip, and a planted one-ulp cutoff that the verdicts dimension catches. The release side is the Phase 5 `trailing_pnl_cutoff` member (see the [Phase 5 guide](rearchitecture_phase5_models.md)) |
...
| — | `_native()` residual | **Closed 2026-09-19** (based on `9916035`). `checks/phase4_real.py::_native()` (the `NativeScoreInputs.from_legacy_fields` wrapper) built inputs for its five side-control call sites (`:734`, `:735`, `:740`, `:774`, `:871` at `50d7a47`). `_application_controls`/`_completion_controls` now build a `SourceBundle` (`_application_control_source`: a real STR-THRU straddle, quotes engineered so native pricing lands on `entry_cost=5.0`) through `build_native_score_inputs`, same as the independence control. `_chooser_controls` builds `ScoreRecord`s directly, since `application._choose_dynamic` is pure selection logic over already-scored records and never touches `NativeScoreInputs` at all -- it never needed `from_legacy_fields` either. Side effect: this also fixes `chooser_no_regating_control`, silently `False` before (the old construction never populated `exp_pnl_model`, so `NO_SCORE` always fired regardless of the property under test). The `_native()` helper is deleted; `NativeScoreInputs.from_legacy_fields` itself stays for its remaining non-acceptance unit-test callers (`tests/test_v2_scoring_application.py`, `tests/test_v2_scoring_application_track_b.py`, `tests/test_v2_scoring_stage_ownership.py`, `tests/test_v2_scoring_no_score_status.py`; `application.py`'s `_frozen_native_inputs` comment updated to stop citing phase4_real.py). No side control needed to keep a legacy-derived input. Tests: `tests/test_phase4_native_side_controls.py` (each converted control's construction proven native-derived; financial values move with real pricing, not copied; a legacy answer field smuggled into the source is refused by `SourceBundle._reject_answers`, not silently accepted) |
