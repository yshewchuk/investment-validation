...
Schema `phase6_capability_matrix.v1.0`: 225 discovered entrypoints, 48 capability rows, 19 excluded tooling entrypoints, 91 adapter edges.

Rows per disposition: native 17, compatibility-adapter 28, CLI-with-user-acceptance-needed 0, missing 1, dormant-historical 2.

Rows per owner: 8A 2, P6-2 15, P6-3 10, P6-4 16, P6-5 4, P6-6 1.

Adapter edges per owner: 8A 2, P3B 12, P4 36, P5 9, P6-2 7, P6-3 9, P6-4 15, P6-5 1.
...
| `nightly-scheduling` | Native shadow nightly scheduling: ORATS-finality-gated trigger, retry window, missed-deadline handling | `cron:dashboard/README.md legacy nightly line (unchanged)` | `cli:engine/v2/ops/nightly_trigger.py`<br>`systemd:ops/systemd/native-nightly-trigger.timer` | systemd timer -> nightly_trigger.run_trigger -> ops plan/submit nightly (shadow) | as_of date; state file reports/phase6/nightly_trigger/<as_of>.json | `tests/test_v2_ops_nightly_trigger.py` | native | P6-6 |
| `nightly-refresh` | Nightly data refresh: calendar, chains, prices, history backfill, computed moves | `nightly:legacy refresh`<br>`nightly:legacy backfill`<br>`nightly:legacy moves`<br>`cli:engine/data/pulls/forward_calendar.py`<br>`cli:engine/data/pulls/computed_moves.py` | `nightly:v2 refresh`<br>`nightly:v2 materialize`<br>`cli:ops price-refresh`<br>`cli:ops price-history`<br>`cli:ops price-history capture`<br>`cli:ops snapshot`<br>`cli:ops snapshot plan-import`<br>`cli:ops snapshot submit`<br>`cli:ops snapshot promote`<br>`cli:ops snapshot rollback`<br>`cli:ops capture-inputs`<br>`nightly:v2 computed_moves_refresh`<br>`nightly:v2 forward_calendar_refresh` | incremental_refresh job (opt-in --refresh-mode native), the native computed_moves_refresh/forward_calendar_refresh job kinds, and snapshot import/promote; default refresh_mode stays legacy | snapshot manifest id / promoted snapshot head; RefreshPlan document hash | `tests/test_v2_ops_incremental_refresh_wiring.py`<br>`tests/test_v2_ops_snapshot_stages.py`<br>`tests/test_v2_ops_price_history.py`<br>`tests/test_data_price_refresh.py`<br>`tests/test_computed_moves.py`<br>`tests/test_v2_ops_calendar_moves_jobs.py`<br>`tests/test_v2_ops_native_computed_moves_forward_calendar_e2e.py` | compatibility-adapter | P6-2 |
| `nightly-features` | Tier-3 panel and Tier-4 forecast rebuild before scoring | `nightly:legacy tiers`<br>`cli:engine/data/rebuild.py`<br>`cli:engine/data/features/tier4.py` | `nightly:v2 features`<br>`action:legacy_features` | legacy_features action (engine.data.rebuild panel+tier4 build) inside a supervised worker, in the real DAG between finality and score | features.json receipt records panel_sha256/tier4_sha256 from the build that just ran; legacy_score binds it (nightly.py _legacy_params) and refuses FEATURES_MISSING with no receipt, FEATURES_STALE on a hash mismatch, checked unconditionally at the top of _action_score (not gated behind snapshot input_mode, since the real default nightly plan is input_mode="legacy" and never reaches snapshot_stages.prepare_launch) | `tests/test_tier4.py`<br>`tests/test_panel.py`<br>`tests/test_v2_ops_legacy_features.py` | compatibility-adapter | P6-2 |
...
| `decisions-supersede` | Reasoned superseding entry for an immutable bad decision | `writer:engine/ledger.py::supersede` | `cli:ops decisions`<br>`cli:ops decisions supersede`<br>`job:decisions_supersede`<br>`py:engine/v2/ops/decision_commit.py::commit_supersede` | ops decisions supersede CLI -> decisions_supersede job -> decision_commit.commit_supersede | decision_id chain | `tests/test_v2_ops_authority.py`<br>`tests/test_ledger.py` | native | P6-3 |
...
| `research-tools` | Research data tools: real-fill pull and quality, trade table build/prune, signal screen, log diagnostics | `cli:engine/data/pulls/polygon_fills.py`<br>`cli:tools/fill_quality.py`<br>`cli:engine/build_trades.py`<br>`cli:tools/reconcile_trades.py`<br>`cli:tools/signal_screen.py`<br>`cli:tools/log_diagnostics.py` | `cli:tools/v2_fill_quality.py`<br>`cli:tools/v2_polygon_fills.py`<br>`cli:tools/v2_signal_screen.py` | legacy Tier-2 store readers/writers (engine.data.store) | none: read the mutable legacy store | `tests/test_polygon_fills.py`<br>`tests/test_fill_quality.py`<br>`tests/test_build_trades.py`<br>`tests/test_log_diagnostics.py` | compatibility-adapter | P6-2 |
...
| `strategy-derivation` | Strategy definitions and per-row derivation (gate terms, thresholds, worked example) | `view:legacy #/models/derivation`<br>`control:legacy d-strategy`<br>`control:legacy d-row` | `route:v2-ops GET /derivation`<br>`route:v2-ops GET /derivation.json`<br>`py:engine/v2/registry/strategies.py::default_registry` | native frozen registry (engine/v2/registry/strategies.py) projected by engine/v2/serving/derivation_projection.py and served by the operations server | registry StrategySpec definition_hash per strategy; scope-current, not release-pinned | `tests/test_dashboard.py`<br>`tests/test_v2_registry_scoring.py`<br>`tests/test_v2_serving_derivation.py`<br>`tests/test_v2_dashboard_preview_derivation.py` | native | P6-4 |
...
| `history-analogs` | Historical prints and analog list for a ticker | `route:legacy-app GET /api/ticker/{sym}`<br>`view:legacy #/trades/explorer` | `route:v2-ops GET /analogs`<br>`route:v2-ops GET /analogs.json`<br>`route:v2-ops GET /explorer`<br>`route:v2-ops GET /release/*`<br>`py:engine/v2/serving/analog_projection.py::analog_document` | legacy ticker payload inside the compatibility bundle; native analog row ids persisted on serving_score_summary by the scoring analog stage, projected by engine/v2/serving/analog_projection.py | compatibility bundle inside the published release_id; release_id + event_id for the native analog view (row ids and a count only) | `tests/test_dashboard.py`<br>`tests/test_analogs.py`<br>`tests/test_v2_serving_analogs.py`<br>`tests/test_v2_dashboard_preview_analogs.py` | native | P6-4 |
...
| `operations-health` | Health: ops liveness document, selfcheck result, job and publication health | `route:legacy-app GET /api/health`<br>`view:legacy #/models/health` | `cli:ops health`<br>`cli:ops doctor`<br>`cli:ops init`<br>`route:v2-ops GET /health.json`<br>`route:v2-api GET /api/v1/operations`<br>`route:v2-ops GET /flags` | ops health document (operations_health.v1.0) | health generated_at + release_id; scope-current, not release-pinned | `tests/test_v2_ops_health.py`<br>`tests/test_v2_ops_serving.py` | native | P6-4 |
| `operations-model-calibration-health` | Model/calibration health (brier skill, settlement diagnostics, champion versions) | `route:legacy-app GET /api/health`<br>`view:legacy #/models/health` | `route:v2-ops GET /calibration-health.json`<br>`py:engine/v2/ledger/calibration.py::export_health_file` | decisions-calibration (engine/v2/ledger/calibration.py::calibrate) publishes the immutable ledger_health.v1 artifact; export_health_file copies it to a release-side file | ledger health generated_at + n_scored, bound to the artifact content_hash recorded by health_ref(conn) | `tests/test_v2_ledger_calibration.py`<br>`tests/test_v2_dashboard_preview_ops_root.py` | native | P6-4 |
| `operations-quotas` | Provider quota state and reserve | `py:engine/dashboard/render.py::quota_state` | `py:engine/v2/ops/scheduler.py::_reserve_provider`<br>`route:v2-ops GET /release/*`<br>`cli:ops provider-account` | legacy quota_state adapter inside legacy_render; v2 provider_budget enforces but does not publish | meta.json in the release bundle | `tests/test_v2_ops_provider_admission.py`<br>`tests/test_ledger.py` | compatibility-adapter | P6-4 |
...
| `export-offline-file` | Offline single-file export of a pinned release (phone without network) | `writer:engine/dashboard/render.py::write_single_file` | `cli:tools/v2_dashboard_offline_file.py`<br>`py:tools/v2_dashboard_offline_file.py::export_offline_file` | write_single_file reused byte-for-byte over the real v2 release tree (releases/<release_id>/, the same P3-4 bundle format tools/v2_dashboard_project.py stages), refusing a traversing/multi-segment release id, an indirect (symlinked) release directory, credential-shaped content (engine.dashboard.publish.secret_scan) and any remaining external http(s) reference | pinned release_id; content_hash of the built file | `tests/test_v2_dashboard_offline_file.py`<br>`tests/test_dashboard.py` | compatibility-adapter | P6-5 |
| `export-remote-delivery` | Authenticated remote snapshot for phone access (Cloudflare Access), with access probe | `nightly:legacy publish`<br>`doc:dashboard/README.md` | `py:engine/v2/serving/operations.py::create_server`<br>`py:engine/v2/dashboard/preview.py::run`<br>`nightly:v2 delivery` | engine.v2.serving.operations.create_server: every route (data/release/health) refuses a missing or wrong bearer token, on ANY bind address, gated by engine.v2.dashboard.preview's token-required, loopback-by-default CLI launcher (cli:engine/v2/dashboard/preview.py, already owned by board-compat-shell) | release_id resolved once from the authenticated /release/current.json route | `tests/test_v2_dashboard_preview.py`<br>`tests/test_checks_phase6_remote_access.py` | native | P6-5 |
| `export-restore-drill` | Restore a deployment from backup, replay an original score offline, reconcile ledger/report | `doc:RECOVERY.md` | `cli:tools/v2_restore_drill.py`<br>`py:tools/v2_restore_drill.py::run_drill`<br>`py:engine/v2/ops/backup.py::restore_backup` | tools/v2_restore_drill.py composes three real entrypoints: ops/backup.restore_backup (isolated restore, every database/artifact byte reverified), ops/cli.rescore_command (the same no-fit, no-provider-pull ops-rescore replay of one ScoreRequest/NativeScoreInputs pair restored from the backup's artifacts/), and ledger/export.export_generation over the restored catalog, diffed byte-for-byte against a generation exported from the live catalog before the backup | backup receipt; restore_drill_receipt.v1.0 (score content_hash + ledger export diff + decision count) | `tests/test_v2_restore_drill.py`<br>`tests/test_v2_ops_effects_backup.py`<br>`tests/test_v2_ops_authority.py` | native | P6-5 |

#### MISSING capabilities (the P6-2..P6-5 work)

| Row | Missing capability | Owner | What is missing |
|---|---|---|---|
| `research-replay` | Replay selected trades on real prices through the same kernel as serving | P6-2 | v2 application.replay re-scores a frozen decision; research trade replay on real prices still runs engine.replay. Deferred: no experiments run during the migration (user, 2026-09-19); the runner stays legacy until after cutover. |
...
- `cli:tools/mutation_pilot.py`<br>`cli:tools/mutation_report.py`<br>`cli:tools/mutation_results.py`<br>`cli:tools/gremlin_pilot.py`<br>`cli:tools/gremlin_results.py`: Mutation-testing CI tooling (mutmut and pytest-gremlins backends).
...
- `cli:tools/land.py`: Supervisor merge/gate/push wrapper (hygiene + bounded tests before pushing a branch to main); development tooling, not a board/nightly consumer.
...
| `engine.v2.ops.legacy_adapter::engine.paths` | exact-symbol | declared legacy input manifest (rooted via _rooted_import) | supervised legacy_* nightly job kinds (engine/v2/ops/nightly.py) | P6-2 | phase-4/6 native features extraction |
| `engine.v2.ops.legacy_adapter::engine.data.store` | exact-symbol | declared legacy input manifest (rooted via _rooted_import) | supervised legacy_* nightly job kinds (engine/v2/ops/nightly.py) | P6-2 | phase-4/6 native features extraction |
| `engine.v2.ops.legacy_adapter::engine.data.rebuild` | exact-symbol | declared legacy input manifest (rooted via _rooted_import) | supervised legacy_* nightly job kinds (engine/v2/ops/nightly.py) | P6-2 | phase-4/6 native features extraction |
...
| `tools/v2_dashboard_offline_file.py` | file-format | a pinned v2 release directory's legacy bundle bytes (index.html, assets/, data/), via write_single_file and secret_scan | export-offline-file | P6-5 | - |
