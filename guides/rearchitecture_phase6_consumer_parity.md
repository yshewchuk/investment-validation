...
Schema `phase6_capability_matrix.v1.0`: 188 discovered entrypoints, 46 capability rows, 12 excluded tooling entrypoints, 82 adapter edges.

Rows per disposition: native 6, compatibility-adapter 26, CLI-with-user-acceptance-needed 1, missing 11, dormant-historical 2.

Rows per owner: 8A 2, P6-2 15, P6-3 10, P6-4 15, P6-5 4.

Adapter edges per owner: 8A 2, P3B 12, P4 36, P5 9, P6-2 4, P6-3 4, P6-4 15.
...
| `decisions-calibration` | Regenerate the ledger calibration report and health.json from settled predictions | `cli:engine.ledger calibrate`<br>`writer:engine/ledger.py::calibrate`<br>`writer:engine/ledger.py::write_health` | `MISSING` | legacy engine.ledger.calibrate only | none | `tests/test_ledger.py` | missing | P6-3 |
| `decisions-status` | Ledger status summary (counts, duplicates, pending settlement) | `cli:engine.ledger status` | `MISSING` | legacy engine.ledger.status | none | `tests/test_ledger.py` | missing | P6-3 |
...
| `books-funding` | Book totals, capital per trade and funding computed against canonical records | `cli:engine/portfolio.py` | `MISSING` | legacy engine.portfolio.build_book/summarize only | none | `tests/test_portfolio.py` | missing | P6-3 |
...
| `board-refresh-action` | Desk action: re-run the nightly without publishing | `route:legacy-app POST /api/refresh` | `cli:ops plan`<br>`cli:ops submit` | supervised nightly plan/submit | job ids of the submitted plan | `tests/test_v2_ops_nightly_completion.py` | CLI-with-user-acceptance-needed | P6-4 |
| `board-adhoc-rescore` | Desk action: ad-hoc re-score of one ticker/strategy at an off-ladder strike or expiry | `route:legacy-app GET /api/score` | `MISSING`<br>`py:engine/v2/scoring/application.py::score_one` | legacy engine.score.score in the request process | none (unpinned, in-process scorer) | `tests/test_score.py` | missing | P6-4 |
...
| `models-release-page` | Show the deployed model release (ids, folds, dependencies) matching the scores on the board | - | `MISSING`<br>`py:engine/v2/models/deployment.py::current_release` | Phase 5 deployment pointer; not read by serving | ModelRelease release_id | `tests/test_v2_models_deployment.py` | missing | P6-4 |
...
| `decisions-calibration` | Regenerate the ledger calibration report and health.json from settled predictions | P6-3 | No v2 job recomputes calibration/health from canonical decisions; legacy_render only reads a ledger-generation health artifact. |
| `decisions-status` | Ledger status summary (counts, duplicates, pending settlement) | P6-3 | No ops command summarizes the catalog decisions/outcomes the way engine.ledger status does. |
| `books-funding` | Book totals, capital per trade and funding computed against canonical records | P6-3 | No v2 accounting over catalog decisions; P6-3 must show totals agree with the existing accounting. |
...
| `board-adhoc-rescore` | Desk action: ad-hoc re-score of one ticker/strategy at an off-ladder strike or expiry | P6-4 | score_one exists as a Python callable only; no route, CLI or job returns a result. |
| `models-release-page` | Show the deployed model release (ids, folds, dependencies) matching the scores on the board | P6-4 | System design §11 Evidence/recovery: matching deployment/model page. No v2 view or API reads the deployment pointer. |
...
- `cli:tools/baseline_export.py`<br>`cli:tools/capture_tier0_corpus.py`<br>`cli:tools/replay_tier1.py`: Phase 0/1 baseline and corpus capture/verification tooling; evidence producers, not consumers.
