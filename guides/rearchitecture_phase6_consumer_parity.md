...
Schema `phase6_capability_matrix.v1.0`: 227 discovered entrypoints, 48 capability rows, 23 excluded tooling entrypoints, 91 adapter edges.
...
| `research-replay` | Replay selected trades on real prices through the same kernel as serving | `py:engine/replay.py::replay` | `MISSING`<br>`py:engine/v2/scoring/application.py::replay`<br>`cli:tools/v2_replay.py` | legacy engine.replay | none | `tests/test_replay.py`<br>`tests/test_v2_scoring_application.py` | missing | P6-2 |
| `research-tools` | Research data tools: real-fill pull and quality, trade table build/prune, signal screen, log diagnostics | `cli:engine/data/pulls/polygon_fills.py`<br>`cli:tools/fill_quality.py`<br>`cli:engine/build_trades.py`<br>`cli:tools/reconcile_trades.py`<br>`cli:tools/signal_screen.py`<br>`cli:tools/log_diagnostics.py` | `cli:tools/v2_fill_quality.py`<br>`cli:tools/v2_polygon_fills.py`<br>`cli:tools/v2_signal_screen.py`<br>`cli:tools/v2_build_trades.py`<br>`cli:tools/v2_reconcile_trades.py` | legacy Tier-2 store readers/writers (engine.data.store) | none: read the mutable legacy store | `tests/test_polygon_fills.py`<br>`tests/test_fill_quality.py`<br>`tests/test_build_trades.py`<br>`tests/test_log_diagnostics.py` | compatibility-adapter | P6-2 |
...
- `cli:tools/v2_controlled_failure_drill.py`<br>`cli:tools/v2_resource_measurement.py`<br>`cli:tools/v2_route_probe.py`<br>`cli:tools/v2_session_evidence_check.py`: P6-6 rehearsal tooling: controlled-failure drill, resource-measurement recorder, route probe and session-evidence completeness check; evidence producers and gates, not board/nightly consumers.
