...
Schema `phase6_capability_matrix.v1.0`: 207 discovered entrypoints, 46 capability rows, 19 excluded tooling entrypoints, 91 adapter edges.

Rows per disposition: native 11, compatibility-adapter 31, CLI-with-user-acceptance-needed 0, missing 2, dormant-historical 2.
...
| `research-tools` | Research data tools: real-fill pull and quality, trade table build/prune, signal screen, log diagnostics | P6-2 | Needs a pinned v2 snapshot read path (or a declared frozen legacy-materialized read set) so research reads the official data. Deferred: no experiments run during the migration (user, 2026-09-19); tools stay on the legacy Tier-2 store until after cutover. |
