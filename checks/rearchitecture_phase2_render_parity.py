...
def run_bounded(argv, *, max_rss_gb, cwd, env, poll_s=2, timeout=1800):
    ...
    ten minutes (floored at 60 s), so a job that never gets its slot exits 75
    before ``subprocess.run``'s own deadline can SIGKILL it.
    """
    command = [sys.executable, str(ROOT / "tools" / "bounded_run.py"),
              "--max-rss-gb", str(max_rss_gb), "--poll-s", str(poll_s),
              "--max-wait-s", str(max(60, timeout - 600)), "--", *argv]
