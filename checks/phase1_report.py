...
def build(
    *,
    ...
    return f"""# Phase 1 — Scoring engine: evidence report

Generated by `checks/phase1_report.py`. Every number below comes from the
artifacts named in the provenance block; a report that cannot be regenerated
from its provenance block is a bug.

---

## 1. The replayed trade set

`engine.replay` prices each structure against real ORATS chains at five fill
alphas. This is the analog layer's evidence base, the gate's training target,
and the substrate Phase 2's backtests will use — one code path, so the live
scorer and the research code cannot drift apart.

{trade_set_section()}

The worst→mid→best spread across those columns is the program's headline risk
made visible: it is the same trades, on the same days, differing only in what
was assumed about execution.
...
Predicted win rates graded against realized outcomes, out of sample. The bar is
the base-rate predictor: a model that always forecasts the unconditional win
rate is perfectly calibrated and perfectly useless, so beating its Brier score
is what shows the predictions carry information about *which* trades win.

{calibration_section(calibration)}

---

## 5. Accuracy-evidence checklist

The Phase 4 standards, answered for this phase.

| # | Standard | Status |
|---|---|---|
| 1 | Real traded/quoted prices only | **PASS** — ORATS chain bid/ask via `engine.structures.price_structure`; oquants fitted marks refused at the adapter |
| 2 | Leak audit passed | **PASS** — `assert_causal` runs on every scoring call; the `poison` check confirms it raises rather than warns |
| 3 | Headline numbers walk-forward OOS only | **PASS** — every registry metric comes from `common.walk_forward`; the served model is refit on everything but its published numbers are not |
| 4 | Fill sensitivity shown | **PASS** — every trade priced at α ∈ {{0, .25, .5, .75, 1}}; §1 reports worst/mid/best |
| 5 | Multiple-testing ledger cited | **DEFERRED** — Phase 2 owns the ledger. Phase 1 registered one champion per role with no spec search |
| 6 | Survivorship caveat quantified | **PARTIAL** — the event universe is the ORATS calendar, which includes delisted names; chain coverage (§ provenance) is the binding limit and is reported against the full planned universe |
| 7 | Prediction ledger | **DEFERRED** — Phase 4. The scorer emits everything the ledger needs (`ScoreResult.as_dict`, snapshot hash, model versions) |

---

## 6. Provenance

{provenance_section(replay_report)}
"""
