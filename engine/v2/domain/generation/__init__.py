"""Structure generator template resolution, finite placement search, completeness receipts

Layer 4a of `system_rearchitecture.md` §4.1. Replaces `structures.py`, `forecast_sizing.py`, `fills.py`.

Empty by construction: phase 0 writes no production logic
(`guides/rearchitecture_phase0_baseline.md` §10). See ``README.md`` for what
this package will own, what it deliberately will not, and which packages may
import it.
"""
from .structures import (
    DISABLED,
    STRATEGIES,
    Geometry,
    GeometryRefusal,
    NativeLeg,
    PricedLeg,
    Pricing,
    PricingRefusal,
    generate,
    price,
)

__all__ = [
    "DISABLED", "STRATEGIES", "Geometry", "GeometryRefusal", "NativeLeg",
    "PricedLeg", "Pricing", "PricingRefusal", "generate", "price",
]
