...
| `eod_inventory` | `EODDataset`, `CURRENT_EOD_INVENTORY`, `inventory_by_name`, `inventory_document`, `validate_inventory` — the complete eight-dataset EOD read inventory and its pure completeness checks for incremental planning. |
| `event_revisions` | `EventRevision`, `event_revision_candidate`, `resolve_event_identity`, `apply_event_revision` — stable event-id resolution for date and BMO/AMC corrections; date-only or ambiguous mappings are refused. |
...
| `repository` | `Repository`, `ResolvedSnapshot` — `Repository(conn, store=None)`. `resolve(snapshot_id)` rebuilds a `SnapshotRef` entirely from catalog rows, in one read-only transaction, never from `data_snapshot_heads` (phase-2 guide §8.1, P2-3): it feeds reconstructed rows back through `manifests.fragment_record`/`.dataset_manifest`/`.snapshot_ref` — the same builders `catalog.commit_snapshot` used — and compares the freshly recomputed id/`manifest_hash` against the catalog's own primary key at every level; a mismatch is `MANIFEST_CORRUPT` (this package's judgement call in place of the guide prose's `INTEGRITY_FAILED` — §11 has no such code), an unknown snapshot id is `SNAPSHOT_NOT_FOUND`. `resolve_full(snapshot_id)` returns a `ResolvedSnapshot` (contracts, objects, records and per-table manifests, plus the `SnapshotRef` itself) reconstructed the same way — the mechanism a caller uses to carry every OTHER table's dataset version forward unchanged into a new snapshot commit without re-scanning anything, since every one of these rows is already content-addressed and `catalog._insert_*` accepts an identical payload for an existing id as a no-op (`engine.v2.ops.price_history_store`'s generation commit is the one caller). `latest_dataset_version(contract_id)` finds a table's newest `data_dataset_versions` row independent of any snapshot's own cadence — for a table like `price_history` whose version chain advances on its own schedule. `scan(query, *, table_name)` (P2-4, needs `store`) is a bounded Arrow scan over one table's pinned fragments per §8.2's nine ordered steps: validates via `query.validate_query`, prunes via `query.fragment_may_match`, opens only surviving objects through `objects.verify_object_path`, and merges each fragment's already-sorted rows into one global primary-key order with a streaming `heapq.merge` (never a sort of the whole result) — never a convenience `read_table()`, always batches. `get_event(event_ref, snapshot_ref)`/`get_chain(query, snapshot_ref)`/`get_price_series(query, snapshot_ref)`/`get_close(ticker, date, observation_ceiling, snapshot_ref)` delegate to `events`/`chains`/`price_history_query`. `explain_dependencies(query, *, table_name=None)` (§5.5) names the exact snapshot, dataset versions, fragments, columns, predicates, and estimated/maximum rows a `DataQuery` would touch; a `ChainQuery` or anything else is refused `UNSUPPORTED_CONTRACT` (Phase 3/4 recipe planning, task brief decision 5). |
...
| `price_download_sources` | `NORMALIZED_COLUMNS`, `read_legacy_px_csv`, `read_tier1_body`, `normalize_px_csv`, `normalize_tier1_body` — read-only parsing of the two legacy yfinance price sources into one normalized frame shape. `read_legacy_px_csv` reuses `panel.add_runup_features`'s own read expression verbatim (panel.py:533); `read_tier1_body` reimplements (root-parametrized, unlike the legacy function) `panel._yf_history_from_tier1`'s parse (panel.py:444-474), pinned to byte-for-byte parse equality against it by a test. Enumerating which files/cache entries exist is ops-layer work (`engine.v2.ops.price_history_store`, needs `legacy_adapter.iter_raw_fetch_cache`) — this module only parses bytes/paths already in hand. |
| `price_history` | `PRICE_HISTORY_VALUE_COLUMNS`, `check_not_backdated`, `latest_state`, `diff_retrieval`, `as_of_view` — the pure diff/as-of logic for the bitemporal `price_history` Tier-2 dataset (`ticker, date, close_adj, close_raw, high_raw, retrieved_at, deleted, source_kind, source_hash, capture_id`; design confirmed 2026-09-14, SEND-BACK 2026-09-14: a real `TableContract`, not a private registry — see `price_history_table`). `diff_retrieval` appends a version row only for a new date, a changed value (exact float64 equality, NaN equal to NaN), or a date missing from a full-history retrieval (a tombstone); refuses `CONTRACT_MISMATCH` on a retrieval that starts later than the ticker's live stored history (a partial window). `as_of_view` reconstructs one ticker's series at a cutoff, the branch chosen once for the whole ticker, never per date. `engine.v2.ops.price_history_store` is where captures are actually diffed, published as whole-ticker fragments and committed. |
| `price_history_table` | `PRICE_HISTORY_CONTRACT`, `PRICE_HISTORY_TABLE_NAME` — the registered `TableContract` for `price_history` (primary key `ticker, date, retrieved_at`; partitioned `("ticker",)`). One fragment always covers a whole ticker's history, never a byte-level append: `manifests`'s non-overlapping-fragment invariant is checked within a partition, and a later retrieval can correct an already-past date, which a byte-appended fragment could never do without violating that ordering. |
| `price_history_query` | `get_price_series`, `get_close` — a typed read over one ticker's `price_history` versions (`PriceQuery` in, a `PriceSeriesRow` tuple out), modeled on `chains.get_chain`. Takes a `Repository`/`SnapshotRef` and drives a bounded `Repository.scan` over the pinned `price_history` dataset version, exactly like `get_event`/`get_chain` — exposed as `Repository.get_price_series`/`Repository.get_close`, the package's only sanctioned call path. Refuses `QUERY_NOT_BOUNDED` if `session_date` is after `observation_ceiling`, `CONTRACT_MISMATCH` if the snapshot has no `price_history` table or the ticker has no stored history at all. |

<!-- public-interface: loads_document, decode_document, table_contract_hash, fragment_record, fragment_ref, dataset_manifest, snapshot_ref, verify_fragment_record, verify_dataset_manifest, verify_snapshot_ref, fragment_identity_payload, dataset_version_identity_payload, snapshot_identity_payload, build_legacy_mapping, LegacyMappingError, OWNER, MIGRATIONS, publish_legacy_file, inspect_fragment, FragmentInspection, verify_object_path, commit_snapshot, record_failed_import, move_head, validate_query, fragment_may_match, row_matches, order_key, arrow_type_for, null_array_for, ARROW_TYPES, get_event, map_row, security_id_for_ticker, TABLE_NAME, EODDataset, CURRENT_EOD_INVENTORY, inventory_by_name, inventory_document, validate_inventory, EventRevision, event_revision_candidate, resolve_event_identity, apply_event_revision, get_chain, contract_id_for, exact_decimal_string, QUOTE_POLICY_REF, Repository, ResolvedSnapshot, DataError, computed_moves, computed_moves_table, NORMALIZED_COLUMNS, read_legacy_px_csv, read_tier1_body, normalize_px_csv, normalize_tier1_body, PRICE_HISTORY_VALUE_COLUMNS, check_not_backdated, latest_state, diff_retrieval, as_of_view, get_price_series, get_close -->
