"""Training scripts for the registry's champions.
    ...
    runup_move.py   GBM → absolute stock move from T-14 to T-1
    iv_crush.py     GBM → change in 30-day IV across the print
    gate.py         GBM → per-trade mid-fill return (the selection signal)
    train_all.py    train, evaluate, and register all champions
