#!/usr/bin/env python3
"""EXP-176: grouped raw and volatility-scaled dislocation arms."""
...
BASE = tuple(mod.BASE)
ARM_FEATURES = {
    "raw_trio": ("abs_ret5", "abs_ret10", "abs_ret20"),
    "scaled_trio": ("abs_ret5_over_rvol30", "abs_ret10_over_rvol30", "abs_ret20_over_rvol30"),
    "all_six": ("abs_ret5", "abs_ret10", "abs_ret20", "abs_ret5_over_rvol30", "abs_ret10_over_rvol30", "abs_ret20_over_rvol30"),
}
PRIMARY = "all_six"


def log(message: str) -> None:
    print(f"[EXP-176 {time.monotonic() - STARTED:,.0f}s] {message}", flush=True)
...
def annual_delta(base: pd.DataFrame, cand: pd.DataFrame) -> tuple[list[dict], np.ndarray]:
    ...
    for year in sorted(base["year"].unique()):
        bpart = base[base["year"] == year]
        cpart = cand[cand["year"] == year]
...
def run_accuracy(data: pd.DataFrame) -> tuple[dict, dict, pd.DataFrame]:
    ...
    for seed in SEEDS:
        for name, features in arms.items():
            frame = mod.fit_oos(data, features, seed, name)
            if seed == SEEDS[0]:
    ...
    for name in ARM_FEATURES:
        ...
        try:
            ...
            pvalue = None
        out[name] = {
            "baseline": {**mod.regression_metrics(base["abs_move"], base["pred"]), **rank_metrics(base)},
            "challenger": {**mod.regression_metrics(cand["abs_move"], cand["pred"]), **rank_metrics(cand)},
            "years_mae_improved": int((deltas < 0).sum()), "wilcoxon_p": pvalue, "by_year": by_year,
        }
    seed_deltas = {}
    for seed in SEEDS:
        rows = [r for r in records if r["seed"] == seed]
        bmae = next(r["mae"] for r in rows if r["arm"] == "baseline")
        seed_deltas[str(seed)] = {name: float(next(r["mae"] for r in rows if r["arm"] == name) - bmae) for name in ARM_FEATURES}
    return {"arms": out, "rows": int(len(base))}, {"records": records, "mae_delta_vs_baseline": seed_deltas}, first_frames[PRIMARY]
...
def extra_sections(accuracy: dict, seeds: dict) -> list[dict]:
    ...
    return [
        {"title": "Joint dislocation-feature arms", "body": ["The primary all_six arm receives the raw and rVol30-scaled 5-, 10-, and 20-session dislocations together. All arms use identical union-complete rows."]},
        {"title": "Matched OOS accuracy", "columns": ["arm", "MAE", "RMSE", "r", "years improved", "Wilcoxon p"], "align": ["---"] * 6, "rows": rows},
        {"title": "Five-seed MAE deltas versus baseline", "columns": ["seed", *ARM_FEATURES.keys()], "align": ["---"] * 4, "rows": seed_rows},
    ]
...
def main() -> None:
    ...
    gate = mod.mod.Gate(fit=gate_state.fit, select=gate_state.select, name="size_joint_dislocation")
    result = mod.mod.evaluate(
