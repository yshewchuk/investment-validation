...
def main(record: bool = True) -> None:
    ...
    if record:
        ...
        if lib.spec_hash(spec) not in already:
            lib.record_evaluation(HERE, spec, {"headline": {
                "mean": results[key].get("on_capital_pct"),
                "sharpe_trade": results[key].get("cagr_pct")}})
            print("[EXP-136] ledger row recorded", flush=True)
